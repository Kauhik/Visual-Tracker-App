...
enum ProgressCalculator {
    ...
    static func objectivePercentage(student: Student, objectiveCode: String, allObjectives: [LearningObjective]) -> Int {
        guard let objective = allObjectives.first(where: { $0.code == objectiveCode }) else {
            return student.completionPercentage(for: objectiveCode)
        }
        return objectivePercentage(student: student, objective: objective, allObjectives: allObjectives)
    }
    ...
    static func cohortObjectiveAverage(objectiveCode: String, students: [Student], allObjectives: [LearningObjective]) -> Int {
        guard let objective = allObjectives.first(where: { $0.code == objectiveCode }) else { return 0 }
        return cohortObjectiveAverage(objective: objective, students: students, allObjectives: allObjectives)
    }
    ...
    static func groupOverall(group: CohortGroup, students: [Student], allObjectives: [LearningObjective]) -> Int {
        groupOverall(group: group, students: students, memberships: [], allObjectives: allObjectives)
    }
}
